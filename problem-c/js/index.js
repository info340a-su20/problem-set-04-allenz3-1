...
function extractOpponent(game) {
  if (game.home === "UW") { //UW is home team
    return game.opponent;
  } else { //UW is opponent team
    return game.home;
  }
}
...
const UWOpponents = huskyGames2016.map(extractOpponent);
...
function huskiesLost(game) {
  if(game.opponent !== "UW") { //UW is home team
...
const fumbleGames = huskyGames2016.filter(game => {
  return game.fumbles > 0;
}).length;
console.log(fumbleGames);
...
function mostYardsPassing(game1, game2) {
  if (game1.passing_yards === null) {
    return game2;
  } else if (game1.passing_yards > game2.passing_yards) {
    return game1;
  } else { //game1.passing_yards < game2.passing_yards
    return game2;
  }
}
...
const fumbledAndLostFilter = makeCombinedFilter(huskiesLost, game => {
  return game.fumbles > 0;
});
